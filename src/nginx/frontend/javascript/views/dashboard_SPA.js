...
export function loadUserManagementData()
{
	fetch('login/api/getData') //TODO adapter a path et nom fonction jess
		.then(response =>
		{
			if (!response.ok)
				throw new Error('Error : network response');
...
// export function ChartBarData(statsData)
// {
...
export function Avatars(statsData, userData)
{
...
export function displayGameHistory(connectedUser, chosenOpponent, userData)
{
...
export function addGameHistory(connectedUser, chosenOpponent, userData)
{
...
export function ChartDoughnutData(statsData)
{
...
export function Badge(statsData) {
	document.getElementById('trophee_icon').addEventListener('click', function() {
		let badgeSrc = '';
		let message = '';
...
//----------------------------------------- test
export function test(statsData) {
    if (!statsData) {
        console.log('statsData is undefined');
        return;
    }
    // Assuming statsData has a property 'nickname'
    console.log(statsData.nickname); // Logs the nickname to the console

    // Find the div by id and set its text content to the nickname
    const nicknameDisplay = document.getElementById('nicknameDisplay');
    if (nicknameDisplay) {
        nicknameDisplay.textContent = `Nickname: ${statsData.nickname}`;
    } else {
        console.log('Div with id "nicknameDisplay" not found. Please add it to index.html.');
    }
}