...
async function profileEditMode(userData_edit, personalInfoSection)
{
    ...
    saveButton.addEventListener('click', (event) =>
    {
        ...
        // Get avatar data
        const data = new FormData(avatarLabel);
        const avatarFile = data.get('avatarInput');

        const isValidAvatar = verifyAvatarFile(avatarFile);
        const isValidData = verifyProfileChanges();

        if (DEBUG)
            console.log('Updated values are valid:', isValidData);

        if (isValidData && isValidAvatar)
        {
...
{
    const avatarFileType = document.getElementById('first_name');
    const avatarTypeType = document.getElementById('first_name');
    const avatarSizeType = document.getElementById('first_name');

    const errorMessages = {
        avatar_file: 'Error: File is empty or does not exist',
        avatar_type: 'Error: File type not supported',
        avatar_size: 'Error: File too large (max 1 MB)',
    };

    const fields = [
        { type: avatar_file_type, id: 'file', message: errorMessages.avatar_file },
        { type: avatar_type_type, id: 'type', message: errorMessages.avatar_type },
        { type: avatar_file_type, id: 'size', message: errorMessages.avatar_size },
    ];
