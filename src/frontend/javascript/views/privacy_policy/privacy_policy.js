...
export function renderPrivacyPolicy()
{
	...
	title.classList.add('priv-title', 'text-center');
	title.textContent = 'Privacy Policy';
	policyContainer.appendChild(title);

	// Create the effective date
	const	effectiveDate = document.createElement('p');
	effectiveDate.classList.add('text-center', 'priv-body');
	const	today = new Date();
	effectiveDate.textContent = `Effective Date: ${today.toLocaleDateString('en-US',
	{
		month: 'long',
		day: '2-digit',
		year: 'numeric'
	})}`;
	policyContainer.appendChild(effectiveDate);

	// Render policy text with breaks
	...
	paragraphs.forEach(paragraphText =>
	{
		if (paragraphText.trim())
		{
			...
			paragraph.classList.add('priv-body');
			
			const	linesInParagraph = paragraphText.split('\n');
			linesInParagraph.forEach((text, index) =>
			{
				paragraph.appendChild(document.createTextNode(text.trim()));
				if (index < linesInParagraph.length - 1)
				{
					paragraph.appendChild(document.createElement('br'));
				}
			});

			policyContainer.appendChild(paragraph);
		}
	});
